...
{
    public class horizontalgrid_line_store
    {
        ...
        public List<meshdata_store> horizontalgrid_inside_lines = new List<meshdata_store>();
        public List<meshdata_store> horizontalgrid_outside_lines = new List<meshdata_store>();
        ...
        public horizontalgrid_line_store(double centerradius)
        {
            ...
            {
                // Horizontal grid lines including the origin (inside the circle)
                double y_val = -unitcircleradius + (i * grid_interval);


                // initialize the temporary mesh data store to add to the list
                meshdata_store temp_horizontalgrid_line = new meshdata_store();

                // Horizontal grid line inside start and end point
                Vector2 temp_startpt = new Vector2(-(float)boundary_size,
                    (float)y_val);

                Vector2 temp_endpt = new Vector2((float)boundary_size,
                    (float)y_val);
                ...
                for (int j = 0; j < segment_count; j++)
                {
                    ...
                    temp_horizontalgrid_line.add_mesh_point(pt_index1, x, y, 0.0, -1);
                    ...
                    temp_horizontalgrid_line.add_mesh_point(pt_index2, x, y, 0.0, -1);


                    // Set the line index
                    temp_horizontalgrid_line.add_mesh_lines(j, pt_index1, pt_index2, -3);

                }

                // Create the shaders and buffers

                temp_horizontalgrid_line.set_shader();
                temp_horizontalgrid_line.set_buffer();

                // Add to the list
                horizontalgrid_inside_lines.Add(temp_horizontalgrid_line);
            ...
            {
                // Horizontal grid lines (outside the circle)
                double y_val = -(unitcircleradius + ((i + 1) * grid_interval));

                // initialize the temporary mesh data store to add to the list
                meshdata_store temp_horizontalgrid_line1 = new meshdata_store();
                ...
                for (int j = 0; j < segment_count; j++)
                {
                    ...
                    temp_horizontalgrid_line1.add_mesh_point(pt_index1, x, y, 0.0, -1);
                    ...
                    temp_horizontalgrid_line1.add_mesh_point(pt_index2, x, y, 0.0, -1);


                    // Set the line index
                    temp_horizontalgrid_line1.add_mesh_lines(j, pt_index1, pt_index2, -3);

                }

                // Create the shaders and buffers

                temp_horizontalgrid_line1.set_shader();
                temp_horizontalgrid_line1.set_buffer();

                // Add to the list
                horizontalgrid_outside_lines.Add(temp_horizontalgrid_line1);


                y_val = (unitcircleradius + ((i + 1) * grid_interval));

                // initialize the temporary mesh data store to add to the list
                meshdata_store temp_horizontalgrid_line2 = new meshdata_store();

                // Vertical grid line inside start and end point
                temp_startpt = new Vector2(-(float)boundary_size,
                    (float)y_val);

                temp_endpt = new Vector2((float)boundary_size,
                    (float)y_val);
                ...
                for (int j = 0; j < segment_count; j++)
                {
                    ...
                    temp_horizontalgrid_line2.add_mesh_point(pt_index1, x, y, 0.0, -1);
                    ...
                    temp_horizontalgrid_line2.add_mesh_point(pt_index2, x, y, 0.0, -1);


                    // Set the line index
                    temp_horizontalgrid_line2.add_mesh_lines(j, pt_index1, pt_index2, -3);

                }

                // Create the shaders and buffers

                temp_horizontalgrid_line2.set_shader();
                temp_horizontalgrid_line2.set_buffer();

                // Add to the list
                horizontalgrid_outside_lines.Add(temp_horizontalgrid_line2);
        ...
        public void paint_horizontal_grid_lines()
        {

            gvariables_static.LineWidth = 1.0f;

            // Paint the inside circle horizontal grid lines
            for (int i = 0; i < horizontalgrid_inside_lines.Count; i++)
            {
                horizontalgrid_inside_lines[i].paint_dynamic_mesh_lines();

            }

            // Paint the outside circle horizontal grid lines
            for (int i = 0; i < horizontalgrid_outside_lines.Count; i++)
            {
                horizontalgrid_outside_lines[i].paint_dynamic_mesh_lines();
            ...
            drawing_events graphic_events_control)
        {
            ...
            // Update the buffer of inside & outside circle horizontal grid lines

            for (int i = 0; i < horizontalgrid_inside_lines.Count; i++)
            {
                horizontalgrid_inside_lines[i].update_openTK_uniforms(set_modelmatrix, set_viewmatrix, set_transparency,
            ...
            for (int i = 0; i < horizontalgrid_outside_lines.Count; i++)
            {
                horizontalgrid_outside_lines[i].update_openTK_uniforms(set_modelmatrix, set_viewmatrix, set_transparency,
                    graphic_events_control.projectionMatrix,
                graphic_events_control.modelMatrix, graphic_events_control.viewMatrix,
                graphic_events_control.geom_transparency);
            ...
            if (new_CenterPt == Vector2.Zero)
            {
                ...
                for (int i = 0; i < num_inside_grid_lines; i++)
                {
                    ...
                    for (int j = 0; j < segment_count; j++)
                    {
                        ...
                        horizontalgrid_inside_lines[i].update_mesh_point(pt_index1, x, y, 0.0, -1);
                        ...
                        horizontalgrid_inside_lines[i].update_mesh_point(pt_index2, x, y, 0.0, -1);
                ...
                for (int i = 0; i < (int)(num_outside_grid_lines / 2.0f); i++)
                {
                    ...
                    //__________________________________________________________________________
                    // Create the Horizontal grid lines
                    // Add the boundary points for Horizontal grid lines
                    double param_t, x, y;
                    int pt_index1, pt_index2;
                    ...
                    for (int j = 0; j < segment_count; j++)
                    {
                        ...
                        horizontalgrid_outside_lines[(2 * i) + 0].update_mesh_point(pt_index1, x, y, 0.0, -1);
                        ...
                        horizontalgrid_outside_lines[(2 * i) + 0].update_mesh_point(pt_index2, x, y, 0.0, -1);
                    ...
                    //__________________________________________________________________________
                    // Create the Horizontal grid lines
                    // Add the boundary points for Horizontal lines
                    ...
                    for (int j = 0; j < segment_count; j++)
                    {
                        ...
                        horizontalgrid_outside_lines[(2 * i) + 1].update_mesh_point(pt_index1, x, y, 0.0, -1);
                        ...
                        horizontalgrid_outside_lines[(2 * i) + 1].update_mesh_point(pt_index2, x, y, 0.0, -1);
            ...
            else
            {
                ...
                for (int i = 0; i < num_inside_grid_lines; i++)
                {
                    ...
                    if (IsPointsCollinear(temp_startpt, temp_endpt, new_CenterPt) == false)
                    {
                        ...
                        Vector2 temp_midpt = new Vector2(0.0f, (float)y_val);
                        ...
                        for (int j = 0; j < segment_count; j++)
                        {
                            ...
                            // First point 
                            pt_index1 = (2 * j) + 0;
                            horizontalgrid_inside_lines[i].update_mesh_point(pt_index1,
                                ClippedCirclePts[pt_index1].X, ClippedCirclePts[pt_index1].Y, 0.0, normalized_defl_scale);

                            // Second point 
                            pt_index2 = (2 * j) + 1;
                            horizontalgrid_inside_lines[i].update_mesh_point(pt_index2,
                ...
                for (int i = 0; i < (int)(num_outside_grid_lines / 2.0f); i++)
                {
                    ...
                    if (IsPointsCollinear(temp_startpt, temp_endpt, new_CenterPt) == false)
                    {
                        ...
                        for (int j = 0; j < segment_count; j++)
                        {
                            ...
                            // First point 
                            pt_index1 = (2 * j) + 0;
                            horizontalgrid_outside_lines[(2 * i) + 0].update_mesh_point(pt_index1,
                                ClippedCirclePts[pt_index1].X, ClippedCirclePts[pt_index1].Y, 0.0, normalized_defl_scale);

                            // Second point 
                            pt_index2 = (2 * j) + 1;
                            horizontalgrid_outside_lines[(2 * i) + 0].update_mesh_point(pt_index2,
                                ClippedCirclePts[pt_index2].X, ClippedCirclePts[pt_index2].Y, 0.0, normalized_defl_scale);
                    ...
                    if (IsPointsCollinear(temp_startpt, temp_endpt, new_CenterPt) == false)
                    {
                        ...
                        for (int j = 0; j < segment_count; j++)
                        {
                            ...
                            // First point 
                            pt_index1 = (2 * j) + 0;
                            horizontalgrid_outside_lines[(2 * i) + 1].update_mesh_point(pt_index1,
                                ClippedCirclePts[pt_index1].X, ClippedCirclePts[pt_index1].Y, 0.0, normalized_defl_scale);

                            // Second point 
                            pt_index2 = (2 * j) + 1;
                            horizontalgrid_outside_lines[(2 * i) + 1].update_mesh_point(pt_index2,
                                ClippedCirclePts[pt_index2].X, ClippedCirclePts[pt_index2].Y, 0.0, normalized_defl_scale);
